...
basepath = "Z:/SIHAQII/data/MODIS_atmprofiles/MOD07_L2/" # set main working directory - CHANGE 
csvpath = "Z:/SIHAQII/data/input_CSVs/" # path to the folder with the csv file containing the smoke plume event info - CHANGE
...
# # FOR SOFTWARE RELEASE TESTING ONLY:
# basepath = 'Z:/SIHAQII/data/Testfiles_softwarerelease/MOD07/' # CHANGE LOCAL PATH
# csvpath = 'Z:/SIHAQII/data/Testfiles_softwarerelease/input_csvs/' # CHANGE LOCAL PATH
...
# for idx, row in subset_df.iterrows(): # toggle this line to use a subset of the whole dataframe
    ...
    if os.path.exists(filepath): # if the file exists
        ...
        plotspath = 'Z:/SIHAQII/data/MODIS_atmprofiles/Outputs/plots/'+str(date)+'_J'+str(day)+'_'+str(filetime)+'/'
        ...
        if len(MHs) > 0: # if MHs were calculated over the plume
            ...
            MH_df.to_csv(path_or_buf='Z:/SIHAQII/data/MODIS_atmprofiles/Outputs/mixingheights/'+str(date)+'_J'+str(day)+'_'+str(filetime)+'.csv')
            MH_df.head()
        else:
...
MHcsvpath = 'Z:/SIHAQII/data/MODIS_atmprofiles/Outputs/mixingheights/' # set path to the output csv
...
for csvfile in os.listdir(MHcsvpath): # loop through all the csv files produced
    if csvfile.startswith('2'): # do not account for other files, all should start with 2 because of the year
